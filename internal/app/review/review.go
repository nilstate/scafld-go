...
func Run(ctx context.Context, specs SpecStore, sessions SessionStore, clock Clock, taskID string) (Output, error) {
	...
	_, err = sessions.Append(ctx, model.TaskID, session.Entry{Type: "review", Status: "pass", Reason: "review gate passed"}, now)
	return Output{TaskID: model.TaskID, Verdict: "pass"}, err
}
